// ==========================================================================
// LEGALBOT AI - MOTOR FIABLE SIN BLOQUEOS
// ==========================================================================
...
// Las 29 preguntas oficiales del PDF
const preguntas = {
  ...
  6: "Objeto principal de la colaboración tecnológica (Describa el software o plataforma brevemente, ej: 'Diseño, desarrollo y operación de la plataforma PAYBRIDGE 360'):",
  7: "Fecha estimada de inicio para la ejecución del proyecto (Ej: DD/MM/AAAA o De inmediato):",
  8: "Plazo de duración estimado para la Fase Inicial de Desarrollo:",
  9: "Plazo de duración estimado para la Fase de Pruebas Técnicas:",
  10: "Plazo de duración estimado para la Fase de Modificaciones y Ajustes:",
...
// Función global que se activa al dar clic al botón de la página o presionar Enter
function enviar() {
  const input = document.getElementById('respuesta');
  if (!input) {
    alert("Error crítico: No se encontró la caja de texto con el id='respuesta'");
    return;
  }
  ...
  // 1. Mostrar el mensaje del usuario de forma inmediata en el chat
  ...
  // 2. Limpiar la caja de texto para la siguiente respuesta
  input.value = '';

  // 3. Procesar la respuesta con el motor lógico
  ...
  if (state.preguntaActual === 1) {
    if (limpio.includes('si') || limpio.includes('sí') || limpio.includes('acepto')) {
      ...
      agregarBot(`🧠 <b>IA Analizadora:</b> Consentimiento registrado. Iniciando fase corporativa.\n\n<b>Pregunta 2 de 29:</b> ${preguntas[2]}`);
    } else {
      agregarBot('⚠️ <b>ERROR:</b> Para proceder es obligatorio aceptar los términos legales escribiendo "Sí".');
    }
    return;
  }
...
// Escuchar la tecla Enter de forma nativa e inmediata
...
document.addEventListener('DOMContentLoaded', function() {
  agregarBot(
    ...
    `Módulo de analítica basado en el modelo estructurado de desarrollo de software.\n\n` +
    `<b>Pregunta 1 de 29:</b> ${preguntas[1]}`
  );
});

// Generador de Contrato basado en el archivo .docx aportado
function generarContratoIA() {
  ...
  doc += `------------------------------------------------------------------------\n\n`;
  doc += `En la ciudad de Bogotá, D.C., de mutuo acuerdo y con plena capacidad legal comparecen:\n\n`;
  doc += `COMPARECEN:\n\n`;
  ...
  doc += `MANIFIESTAN:\n\n`;
  doc += `I.- Que las entidades intervinientes, conformando un total de (${d.pregunta_2 || '2'}) partes, ostentan objetivos comunes.\n`;
  doc += `II.- Que acuerdan el desarrollo técnico bajo las siguientes:\n\n`;
  doc += `CLÁUSULAS\n\n`;
  
  doc += `PRIMERA.- OBJETO DEL CONTRATO: Regular el desarrollo de la plataforma: "${d.pregunta_6 || '___________'}".\n\n`;
  doc += `SEGUNDA.- HITOS Y FASES DE EJECUCIÓN\n`;
  doc += `- Fecha de Inicio: ${d.pregunta_7 || '___________'}\n`;
  doc += `- Desarrollo
